...
public class CurrentEmp {
    ...
    public void EmployeesTableSchool(JTable table, String valueToSearch) {
        ...
        try {
            ps = con.prepareStatement("SELECT * FROM school_employees WHERE CONCAT (phone)LIKE ? and school_employees.status='Working'");
            ...
            Logger.getLogger(CurrentEmp.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
        ...
        PreparedStatement ps;
        try {
            ps = con.prepareStatement("SELECT * FROM uni_employees WHERE CONCAT (phone)LIKE ? and uni_employees.status='Working'");
            ps.setString(1, "%" + valueToSearch + "%");
